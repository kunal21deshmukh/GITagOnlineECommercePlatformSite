...
import './UpdateSubscriberForm.css';
...
const UpdateSubscriberForm=()=> {
  ...
  const UpdateSubscriber = (e) => {
    ...
    //const subscriber = {subscrEmail,subscrPhoneno,city,state,country,zipCode,location,sid,subscrName};
    ...
    .updateSubInfo(accnt)
    .then((response) => {
      console.log("subscriber details updated successfully", response.data);
      history.push("/accountInfoSub/:id");
    })
    .catch((error) => {
      console.log("something went wrong", error);
    });
...
<form>
     ...
     <tr><td>Name:</td><td className="textBox">
      ...
      <center>
       <button onClick={(e) => UpdateSubscriber(e)} className="btn btn-primary">
         Submit
       </button>
       </center>
